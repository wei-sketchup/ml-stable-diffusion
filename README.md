# Core ML Stable Diffusion (WWDC24)
...
## <a name="wwdc24"></a> WWDC24 changes

<details>
  <summary> Click to expand </summary>

This branch updates this project with some of the features announced at WWDC24, specifically, adopts the [`MLTensor`](https://developer.apple.com/documentation/coreml/mltensor) type for handling out-of-model computation, such as scheduling, and adds support for [LoRA (Low Rank adapters)](https://huggingface.co/docs/transformers/en/peft) from Hugging Face using [multi-function models](https://apple.github.io/coremltools/docs-guides/source/multifunction-models.html).

Check out the relevant WWDC videos to learn more about each of these features.
- [Bring your machine learning and AI models to Apple silicon](https://developer.apple.com/videos/play/wwdc2024/10159/)
- [Deploy machine learning and AI models on-device with Core ML](https://developer.apple.com/videos/play/wwdc2024/10161/)

In the remaining of this section, we walk through an example of converting and running Stable Diffusion with adapters.

**Step 1. Convert the base model and dependents** Run the following command to export Stable Diffusion XL for running an end-to-end diffusion pipeline.

```bash
python -m python_coreml_stable_diffusion.torch2coreml --convert-unet --convert-vae-decoder --convert-text-encoder --xl-version --model-version stabilityai/stable-diffusion-xl-base-1.0 --attention-implementation ORIGINAL --bundle-resources-for-swift-cli -o <output-base-mlpackages-directory>
```

On completion, the 4 neural network models that comprise Stable Diffusion will have been converted from PyTorch to Core ML (.mlpackage) and saved into the specified directory, `<output-base-mlpackages-directory>`.
 With the `--bundle-resources-for-swift-cli` flag set, the 4 models will also be compiled and copied to `<output-base-mlpackages-directory>`/Resources, this includes the necessary resources for text tokenization.

**Step 2. Convert an U-net XL model with adapter** Next, run the following command to export the U-net model of Stable Diffusion XL with the specified adapter. Please, enter a different output directory to save the converted U-net with the adapter.

```bash
python -m python_coreml_stable_diffusion.torch2coreml --convert-unet --xl-version --model-version stabilityai/stable-diffusion-xl-base-1.0 --bundle-resources-for-swift-cli --attention-implementation ORIGINAL -o <output-adapter-mlpackages-directory> --lora-model-id <lora-model-id> --lora-weights-file <lora-weights-file>
```

The options `--lora-model-id` and `--lora-weights-file` specify the LoRA model id and associated weights respectively. You can find many pre-trained LoRA models from Hugging Face's [Diffusers Gallery](huggingface-projects/diffusers-gallery).

**Step 3. Merge the base U-net model with the adapted U-net model** Run the following command to merge the two converted U-net models into a single Core ML model, each callable by its respective function. You can indicate the names you want to assign to each of the merged functions - in this example, we are using `original` and `adapter`.

```bash
python -m python_coreml_stable_diffusion.merge_unet_models --model-packages <output-base-mlpackages-directory>/Stable_Diffusion_version_stabilityai_stable-diffusion-xl-base-1.0_unet.mlpackage <output-adapter-mlpackages-directory>/Stable_Diffusion_version_stabilityai_stable-diffusion-xl-base-1.0_unet.mlpackage --function-names original adapter --output-path <output-base-mlpackages-directory>/  --bundle-resources-for-swift-cli
```

Once complete, the U-net models at `<output-base-mlpackages-directory>` and `<output-adapter-mlpackages-directory>` will be merged into a single model and saved to `<output-base-mlpackages-directory>`. Setting `--bundle-resources-for-swift-cli` will compile the merged model and copy it to the Resources directory, overriding the existing compiled U-net model.

> If `--function-names` is omitted, the script tries to infer the function names based on metadata populated in the conversion step, using "original" if no relevant metadata is found. We recommend you use descriptive function names.

**Step 4. Generate images using Swift and specific function** Run the following command to generate images using the adapter model with the Swift CLI.

```bash
swift run StableDiffusionSample <prompt> --resource-path <output-base-mlpackages-directory>/Resources --output-path <output-directory> --compute-units cpuAndGPU --xl --unet-function-name adapter
```
